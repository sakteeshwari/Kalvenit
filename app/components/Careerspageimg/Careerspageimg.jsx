    ...
    <div className="relative text-white bg-[url('/assets/careerpageimg.png')] bg-cover bg-center h-[40vh] w-[] lg:h-screen">
      ...
      <div ref={elementRef} className={`absolute top-14 lg:left-6 lg:top-36 lg:text-2xl text-lg slide-up`}>
        <Link href="/contact" passHref>
          <button
            className={`relative flex items-center space-x-2 text-white font-medium group 
                      hover:rounded-3xl hover:px-3 hover:py-1 transition-all duration-300 overflow-hidden 
                      ${inView ? "slide-up" : ""}`}
          >
            {/* Border animation */}
            <div
              className="absolute inset-0 w-0 overflow-hidden group-hover:w-full h-full bg-transparent border 
                          border-white rounded-3xl transition-all duration-500 ease-in-out opacity-0 group-hover:opacity-100"
            ></div>
