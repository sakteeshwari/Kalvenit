...
const StatsGrid = () => {
    ...
    const stats = [
        { number: "80+", label: "Fortune 500 customers", bg: "bg-gray-900" },
        {
            ...
            darkOverlay: true, // Flag to add dark overlay
        },
        {
            number: "8500+",
        ...
        <div className="lg:flex lg:justify-evenly bg-gray-100">
            <div className="grid grid-cols-2 sm:grid-cols-2 basis-2/5 px-4 lg:pt-11">
                {stats.map((stat, index) => (
                    <div
                        key={index}
                        className={`relative flex flex-col justify-center items-center text-white p-12 ${stat.bg}`} 
                        style={{
                            backgroundImage: stat.bgImage ? `url(${stat.bgImage})` : undefined,
                            backgroundSize: "cover",
                            backgroundPosition: "center",
                        }}
                    >
                        {/* Dark Overlay (only for items with darkOverlay flag) */}
                        {stat.darkOverlay && (
                        ...
                        <h2
                            ref={elementRef}
                            className={`relative z-10 text-3xl font-bold lg:text-4xl ${inView ? "slide-up" : ""}`}
                        >
                            {stat.number}
                        </h2>
                        ...
                        <p
                            ref={elementRef}
                            className={`relative z-10 text-center text-sm mt-2 lg:text-lg ${inView ? "slide-up" : ""}`}
                        >
                            {stat.label}
                        </p>
                    </div>
            ...
            <div className="mt-8 basis-2/5">
                ...
                <p
                    className={`xl:text-lg p-3 text-black tracking-wide leading-relaxed ${
                        inView ? "slide-up" : ""
                    }`} 
                ...
                <div ref={elementRef} className={` p-3 text-lg slide-up`}>
                    <button 
                        ...
                        className={`relative flex items-center space-x-2 text-red-500 font-medium group hover:rounded-3xl hover:px-3 hover:py-1 transition-all duration-300 overflow-hidden ${
                            inView ? "slide-up" : ""
                        }`}
                    >
                        {/* Border animation */}
